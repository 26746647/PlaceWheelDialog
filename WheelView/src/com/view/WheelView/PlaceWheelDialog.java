...
import com.view.WheelView.WheelView.OnWheelChangedListener;
import com.view.WheelView.WheelView.OnWheelScrollListener;
...
public class PlaceWheelDialog extends Dialog {
	...
	private String[] province = new String[30];
	...
	private void setListener() {
		...
		private int[][] citytoarea ={countyOfBeiJing,
				countyOfTianJing,countyOfHeBei,
				countyOfShanXi1,countyOfNeiMengGu,
				countyOfLiaoNing,countyOfJiLin,
				countyOfHeiLongJiang,countyOfShangHai,
				countyOfJiangSu,countyOfZheJiang,
				countyOfAnHui,countyOfFuJian,
				countyOfJiangXi,countyOfShanDong,
				countyOfHeNan,countyOfHuBei,
				countyOfHuNan,countyOfGuangDong,
				countyOfGuangXi,countyOfHaiNan,
				countyOfChongQing,countyOfSiChuan,
				countyOfGuiZhou,countyOfYunNan,
				countyOfXiZang,countyOfShanXi2,
				countyOfGanSu,countyOfQingHai,
				countyOfNingXia,countyOfXinJiang,
				countyOfHongKong,countyOfAoMen,
				countyOfTaiWan
				};
	    ...
	    private void initiWheelView()
	    {
	        ...
	        province = supercontext.getResources().getStringArray(R.array.province);
	      
	        
	        areaWheelView.setAdapter(new ArrayWheelAdapter<String>(supercontext.getResources().getStringArray(
	        		citytoarea[18][2])));
	        areaWheelView.setVisibleItems(5);
	        areaWheelView.setCurrentItem(0);

	        cityWheelView.setAdapter(new ArrayWheelAdapter<String>(supercontext.getResources().getStringArray(
	                city[18])));
				...
				public void onScrollingFinished(WheelView wheel) {
					...
					switch(wheel.getId()){
					 case 2131230726:
						 ...
						 break;
					 case 2131230727:
						 setArea(provinceId,wheel.getCurrentItem());break;
	    ...
	    public static class Builder {
			private final DatePlaceParams P;

			public Builder(Context context) {
				P = new DatePlaceParams(context);
			}
			...
			public PlaceWheelDialog create() {
				final PlaceWheelDialog dialog = new PlaceWheelDialog(
						P.mContext);
				P.apply(dialog);
				return dialog;
			}
...
//	    
		public static class DatePlaceParams {
			public int mIconId;
			...
			public DatePlaceParams(Context context) {
				mContext = context;
				
			};

			public DatePlaceParams(Context context, Calendar calendar) {
				mContext = context;
				this.calendar = calendar;
			}

			public void apply(PlaceWheelDialog dialog) {
				if (mTitle != null) {
...
//					dialog.setCalendar(calendar);
		...
		/**
		 * 
		 * @return getCurrentPlace string
		 */
		public String getPlace()
		{

			int provinceId = getCurrentPrivinecid();
			int areaid = getCurrentAreaid();
			int cityid = getCurrentCityid();
			
			String provicestr= province[provinceId];
			
			String[] citysstr =supercontext.getResources().getStringArray(city[provinceId]);
			
			String citystr = citysstr[cityid];
			
			String[] areasstr =supercontext.getResources().getStringArray(citytoarea[provinceId][cityid]);
		
			String areastr =areasstr[areaid];
			return provicestr+citystr+areastr;
		}
		
		public int getCurrentPrivinecid()
		{
			return provinceWheelView.getCurrentItem();
		}
		
		public int getCurrentCityid()
		{
			return cityWheelView.getCurrentItem();
		}
		
		public int getCurrentAreaid()
		{
			return areaWheelView.getCurrentItem();
		}
}
